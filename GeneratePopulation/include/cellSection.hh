...
//
/// \file cellSection.hh
/// \brief Definition of the B6::CellSection class

#ifndef B6_CELL_SECTION_H
#define B6_CELL_SECTION_H
...
public:
	void fill() override {
		...
		double minRadiusN = nucleusRadius[0];
		double maxRadiusN = nucleusRadius[1];

		std::vector<double> membraneRadius = this->template load<std::vector<double>>(sectionName, "membraneRadius");
		double minRadiusM = membraneRadius[0];
		double maxRadiusM = membraneRadius[1];
		...
		this->objToFill->SetCellProperties(minRadiusN, maxRadiusN, minRadiusM, maxRadiusM, cytoplasmMaterials, nucleusMaterials);
	}
};
